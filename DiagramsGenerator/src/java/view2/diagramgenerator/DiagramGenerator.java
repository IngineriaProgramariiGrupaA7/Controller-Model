...
public class DiagramGenerator {
    ...
    private void attachHead() {
        String head = null;
        try {
            File file = new File(classLoader.getResource("site/diagram/head.html").getFile());
            head = new String(Files.readAllBytes(file.toPath()), StandardCharsets.UTF_8);
            html.write(head);
            attachNetwork();
        } catch (IOException e) {
            e.printStackTrace();
    ...
    private void createNetwork() throws IOException {
        String network = null;
        try {
            File file = new File(classLoader.getResource("site/diagram/network.js").getFile());
            network = new String(Files.readAllBytes(file.toPath()), StandardCharsets.UTF_8);
            html.write(network);
            html.write("\n\t\t}\n");
    ...
    private void attachBody() throws IOException {
        String body = null;
        try {
            File file = new File(classLoader.getResource("site/diagram/body.html").getFile());
