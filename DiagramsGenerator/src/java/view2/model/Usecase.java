...
public class Usecase {
    ...
    public Usecase(FCUseCase use) {
        this();
        use.getActors().forEach(actor -> actors.add(new Actor(actor, "actors")));
        use.getExtensions().forEach(extension -> actions.add(new Action(extension.getStep(), extension.getDescription())));
        use.getRelationships().forEach(relationship -> edges.add(new Relation(relationship.getEntity_1(), relationship.getEntity_2(), relationship.getRelation())));
    }
